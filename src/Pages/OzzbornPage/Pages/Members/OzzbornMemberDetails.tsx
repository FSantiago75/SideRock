...
import { MEMBER_CARD_ENTER_MS, MEMBER_CARD_EXIT_MS } from './ozzbornMembersMotion'

type OzzbornMemberDetailsProps = {
  member: OzzbornMember
  currentIndex: number
...
export function OzzbornMemberDetails({
  member,
  currentIndex,
  isAutomaticMode,
  isLocked,
  ...
  useLayoutEffect(() => {
    ...
    setDirection(getSwapDirection(fromIndex, currentIndex, OZZBORN_MEMBERS.length))
    setPhase('exit')
      ...
      <div className={styles.detailsTopline}>
        <span className={styles.detailsIndexViewport}>
          <span key={displayed.member.id} className={indexClass}>
            Integrante {formatOzzbornMemberIndex(displayed.currentIndex)}
          </span>
        </span>
        <button
