...
- **ScrollReveal**: `src/Components/ScrollReveal`. Um `IntersectionObserver` (root = `.main` interno). Anima só `opacity`/`transform`. Estado inicial visível (`idle`) se o JS falhar. `prefers-reduced-motion` mostra tudo de imediato. Tokens `--reveal-duration` / `--reveal-distance` / `--reveal-ease` na página. Não animar `width`/`height`/`margin`. Não aplicar `transform` no elemento `position: sticky`.
- **Resumo — linha legal**: `.legalBar` em `BookingSection.module.css`. Barra baixa, padding vertical igual, flex a centrar o texto. Espaço acima (~1.35rem) para não colar nos blocos de contratação; sem padding extra abaixo da linha.
