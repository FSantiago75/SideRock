...
- **Vídeo de contratação**: lista em `bookingContent.ts` (`videos`); o site serve `promotionalVideo*.web.mp4` + posters `.webp`. Os originais `promotionalVideo*.mp4` permanecem no repositório e não são importados. Player nativo; desktop lado a lado; ≤760px carrossel nativo (`overflow-x: auto`, `scroll-snap`, scrollbar oculta, peek do próximo card). Quadro `9 / 16` com `object-fit: cover` e `width`/`height` 1080×1920 no `<video>` — `preload="none"` sem dimensões usa 300×150 e o Chrome/Safari pintam play + barra minúsculos (no telemóvel cortam). Enclosure WebKit a 100%. `min-width: 82%` no card do carrossel para não colapsar. Sem autoplay. `preload="none"` + poster. Ao tocar um, pausar os outros da fila. Não hotlinkar CDN do Instagram. CTA “Ver apresentações” aponta para o perfil (`instagramUrl`). Com `as const`, se todos tiverem `href`, não usar `'href' in video` com ramo else.
- **ScrollReveal**: `src/Components/ScrollReveal`. Um `IntersectionObserver` (root = `.main` interno), `threshold: 0.1`, `rootMargin: 0 0 -6% 0`. A decisão de revelar usa o **rect real** contra o `.main` (não só `isIntersecting` — CSS Columns mente e revela tudo no load). Se o item ainda não entrou, o scroll do `main` volta a medir. Anima só `opacity`/`transform`. Estado inicial visível (`idle`) se o JS falhar. `prefers-reduced-motion` tira o arrasto e faz fade curto — **não** saltar o observer nem revelar pelo viewport com margem positiva. Tokens `--reveal-duration` / `--reveal-distance` / `--reveal-ease` na página. Não animar `width`/`height`/`margin`. Não aplicar `transform` no elemento `position: sticky`. Último bloco do Resumo (faixa de bandas, linha legal) **sem** `ScrollReveal`.
...
- **Integrantes — fundo**: um único asset de atmosfera (`src/assets/sideRock/sideRockMembersAtmosphereV2.png`) atrás da formação. Sem glows radiais extra, sem spotlight a seguir o membro, sem pontos de luz duplicados. Véus só para leitura (escurecer o card à direita).
...
- **Galeria — descoberta automática**: imagens em `src/assets/sideRock/gallery/<album>/`. Cada subpasta é um álbum; prefixos numéricos (`01-shows`) ordenam pastas e ficheiros. `import.meta.glob` com caminho **literal** em `galleryContent.ts`. Sem imports manuais, arrays ou rotas por foto. O `id` da foto é o caminho relativo. Labels: tirar extensão, prefixo numérico e data inicial; `estudio` → `Estúdio`. Pasta vazia = estado “em preparação”, sem quebrar. Novas fotos entram no próximo build.
- **Galeria Ozzborn — espelho isolado**: implementação própria em `src/Pages/OzzbornPage/Pages/Gallery/`; assets exclusivos em `src/assets/ozzborn/gallery/<album>/` via `import.meta.glob` literal em `ozzbornGalleryContent.ts`. Mesma arquitetura da Side Rock (CSS Columns, lightbox em portal, ScrollReveal, estado vazio), mas shell com `OzzbornNavbar`, acento `#7c3aed` / `#c4b5fd`, scrollbar `OZZBORN_SCROLLBAR` e fundo `ozzbornGalleryAtmosphere.png` com overlay roxo leve nas bordas. Nunca importar assets da Side Rock nem reutilizar componentes locais da galeria Side Rock.
- **Repertório Ozzborn — narrativa por eras**: implementação em `src/Pages/OzzbornPage/Pages/Repertoire/`; 13 capas em `src/assets/ozzborn/albums/` (imports explícitos em `ozzbornRepertoireContent.ts`); cinco eras cronológicas com cards conceituais (sem faixas, setlists, players ou dados confidenciais no frontend). Sigilo do repertório executado é requisito de produto e de código entregue ao navegador.
- **Ozzborn — acento no shell compartilhado**: `.navOverlay` e o glow/gradiente do `SectionIntro` (`h1::after`) derivam de `--side-rock-accent` / `--section-intro-accent` via `color-mix` — não hardcodar vermelho nesses pontos. Páginas Ozzborn passam `accent`/`accentHot` no `SideRockSectionPage`; Side Rock mantém o default vermelho sem alteração visual.
- **Galeria — UI**: `layout="flow"` com `src/assets/sideRock/sideRockStandardBackground.png` (névoa vermelha à esquerda). **Sem** grayscale — isso era para a caveira do `BGGaleria.png`. Overlay só para leitura; `.page` transparente para a foto aparecer. `backgroundPosition="left center"` no telemóvel para não perder o glow. Cabeçalho igual ao de Integrantes: eyebrow vermelho (`Arquivo visual`), título serifado `GALERIA`, linha vermelha com `lineReveal`, descrição, contagem de registros e divisor cinza (`rgba(241, 237, 230, 0.1)`) antes do mosaico. Sem filtros de álbum. Grade em CSS Columns com proporção natural (`height: auto`, sem `object-fit: cover`). No telemóvel nunca 1 coluna: 2 colunas desde 320px, 3 entre 400–760px, gutters mais apertados. Desktop inalterado (2 / 3 / 4). O card é uma moldura de arquivo: foto com proporção natural e **caption sempre pintada sobre a foto** (álbum + nome + ícone + filete vermelho). Não depende de hover nem de clique — no telemóvel e no desktop em descanso o título já está visível. Em `(hover: hover) and (pointer: fine)` o hover só aquece borda/glow/escala — **não esconde** a caption. Não colocar a placa abaixo da imagem: retratos altos empurram o título para fora do primeiro viewport. Lightbox em portal no `document.body`, `z-index: 1000`, `object-fit: contain`; setas/Escape/swipe; lock do scroll no `main`; foco restaurado ao card. `ScrollReveal` no cabeçalho (cascata `up`/`none`, como o Resumo) e em cada card (`from="up"`, não `scale`; stagger 70ms). Distância um pouco maior que o Resumo (36px / 24px no telemóvel) para o mosaico se sentir. O observer mede o rect — CSS Columns não pode revelar as fotos de baixo no load. Sem `cardEnter` paralelo. Lightbox **sem** `ScrollReveal`. `prefers-reduced-motion` tira escala/translação dos cards e do reveal (fica fade), não a linha do título.
...
- **2026-09-02**: Galeria Ozzborn — página definitiva em `/ozzborn/galeria`; descoberta automática em `src/assets/ozzborn/gallery/`; fundo `ozzbornGalleryAtmosphere.png`; implementação isolada da Side Rock.
- **2026-09-01**: `src/assets/` — pastas e ficheiros em camelCase (`sideRock/`, `membersImages/`, `sideRockStandardBackground.png`, etc.).
- **2026-09-01**: `src/assets/` reorganizado por escopo (`general/`, `sideRock/`, `acoustic/`, `ozzborn/`); imports atualizados.
- **2026-08-26**: Galeria — fundo `side-rock-standard-background.png` (sem grayscale; página transparente). Contratação — player nativo `cover` + 9/16 intrínseco para a barra/play não cortarem no carrossel.
